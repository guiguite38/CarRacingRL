...
# action_space
# Entre -1.0 et 1.0 en tuple de dimension 3
# (steer, gas, brake)

# observation_space
# L'image : 96x96x3 entre 0 et 255

input_dims = 2
state_placeholder = Input(tf.float32, [None, input_dims]) 
...
def value_function(state):
    ...
    with tf.variable_scope("value_network"):
        hidden1 = layers.Dense(state, n_hidden1, tf.nn.elu)
        hidden2 = layers.Dense(hidden1, n_hidden2, tf.nn.elu) 
        V = layers.Dense(n_outputs, None)
    return V
...
def policy_network(state):
    ...
    with tf.variable_scope("policy_network"):        
        hidden1 = layers.Dense(state, n_hidden1, tf.nn.elu )
        hidden2 = layers.Dense(hidden1, n_hidden2, tf.nn.elu)
        mu = layers.Dense(hidden2, n_outputs, None,)
        sigma = layers.Dense(hidden2, n_outputs, None)
...
import sklearn
import sklearn.preprocessingtf
